...
void KeyCallback(GLFWwindow*window, int key, int scancode, int action, int mode); //关于键盘操作的ESC，和1，2，3，4控制平移
void DoMovement();//方向键控制camera移动
//glm::mat4 try1(glm::mat4 transform);


void MouseCallback(GLFWwindow *window, double xPos, double yPos);//通过鼠标捕获物体调整物体的位置
void ScrollCallback(GLFWwindow *window, double xOffset, double yOffset); //鼠标滚轮调大小作业
void processInput(GLFWwindow* window);//控制ESC和纹理变换
float mixValue=0.0f;//纹理变换控制变量

const GLint WIDTH = 800, HEIGHT = 600; //新建窗口
bool keys[1024]; //保留键盘上的功能的
GLfloat translation1;//x轴平移
GLfloat translation2;//Y轴平移
Camera camera(glm::vec3(0.0f, 0.0f, 4.0f));//放4.0使得人往后挪
...
int main() {
    ...
    glfwSetCursorPosCallback(window, MouseCallback);//传递函数
    ...
    GLfloat vertices[] = {
        /*
         ...
         0.5f,  0.5f,  0.5f,  0.0f, 1.0f, 1.0f,
         -0.5f,  0.5f,  0.5f, 0.0f, 1.0f, 1.0f,
         -0.5f,  0.5f, -0.5f, 0.0f, 1.0f, 1.0f //6个面，每个面2个三角形，一共36个点
    ...
    glGenVertexArrays(2, &VAO);//生成一个对象，存在VAO地址里
    glGenBuffers(2, &VBO);
    //将VAO与VBO绑定
    ...
    unsigned char*image= SOIL_load_image("res/images/image3.jpg", &width, &height, 0, SOIL_LOAD_RGBA);//unsigned 非负数
    ...
    while (!glfwWindowShouldClose(window)) {  //判断窗口是否关闭，若窗口一直开着，则一直刷新屏幕
        ...
        //放在这才有用
        glClearColor(0.2f, 0.3f, 0.3f, 1.0f); //底色--- RGB+透明度（这里设置不透明）
        ...
        glm::mat4 transform = glm::mat4(1.0f);//定义一个4阶矩阵，model矩阵
        ...
        transform = glm::translate(transform, glm::vec3(translation1, translation2, 0.0f));//平移变换
        ...
        glUniformMatrix4fv(transLoc, 1, GL_FALSE, glm::value_ptr(transform));//将位置赋值给他去做变换，可以结合成y下面的一行代码
